...
public class GameLog extends Fenetre implements ActionListener,KeyListener,Runnable {
	...
	protected JPanel questionPanel=new JPanel(new FlowLayout(1,0,15));
	protected Potence p;
	...
	protected JLabel remaining=null;
	...
	protected QuestionPanel imgPanel=new QuestionPanel();
	//default constructor
	public GameLog() {};
	// constructor used in case a game is loaded through a saved game see UserPage
	public GameLog(PlayedGame g,Player joueur) {
	...
	public GameLog(ThemeJeu theme,Player joueur)
	{
		super(800,500,"Eureka");
		this.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
		this.theme=theme;
	...
	protected void addContent() {
		...
		remaining=new JLabel("Il vous reste "+(8-wrongAnswers)+ " coup a jouer");
		...
		remaining.setFont(new Font("arial",Font.PLAIN,16));
		currentAnswer.setFont(new Font("arial",Font.PLAIN,20));
		answer.setFont(new Font("arial",Font.PLAIN,18));
		currentAnswer.setBackground(transparent);
		currentAnswer.setHorizontalAlignment(JLabel.CENTER);
		question.setPreferredSize(new Dimension(380,30));
		currentAnswer.setPreferredSize(new Dimension(380,40));
		...
		JPanel zoneSaisie=new JPanel(new GridLayout(3,1,5,5));
		scoreLabel.setFont(new Font("arial",Font.PLAIN,16));
		potencePanel.add(scoreLabel);
		potencePanel.add(p);
		zoneSaisie.setBackground(transparent);
		zoneSaisie.add(answer);
	...
	public void newQuestion() {
		this.currentQuestion=theme.getQuestion(nbAnswered);
	...
	public void checkAnswer() {
		...
		if(rep.length()==1) {
			...
			if(wrongAnswers==7) {
				...
				remaining.setText("Il vous reste "+(8-wrongAnswers)+" coup a jouer");
			}
		}
	}
		...
		if(this.currentQuestion.getAnswer().toLowerCase().equals(rep.toLowerCase())) {
			...
			if(wrongAnswers==7) {
				...
				//boite de dialogue affichant le score
				joueur.addPlay(new PlayedGame(score,nbAnswered,theme));
				joueur.setLastLevel(nbAnswered);
				new JOptionPane().showMessageDialog(this,"vous avez perdu\n La reponse etait "+currentQuestion.getAnswer()+"\n Votre score est "+score,
						"Game Over",JOptionPane.INFORMATION_MESSAGE);
